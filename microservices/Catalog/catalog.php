...
require_once '../Auth/connection.php'; // Reuse the database connection
...
try {
    ...
    $userId = $_GET['user_id'] ?? null; // Simulated auth token passing for now

    $db = DatabaseConnection::getInstance()->getConnection();

    if ($action === 'list') {
        // Verificar el estado del usuario para decidir si mostrar precios/comprar
        $mostrarPrecio = false;
        if ($userId) {
        ...
        // Validación de Super Usuario omitida por brevedad de ejemplo (reusar logica de Auth en prod)
        $cat = $_POST['categoria'] ?? '';
        $nom = $_POST['nombre'] ?? '';
        $mar = $_POST['marca'] ?? '';
        $sab = $_POST['sabor'] ?? '';
